...
export class ProfessionalCreatePage {
  ...
  loadFormData(): void {
    ...
    forkJoin({
      ...
      error: () => {
        this.error.set('The professional form data could not be loaded.');
        this.loading.set(false);
      },
    });
  }
  ...
  save(data: ProfessionalCreateDto | ProfessionalUpdateDto): void {
    ...
    this.professionalService.create(data as ProfessionalCreateDto).subscribe({
      next: () => {
        this.notificationService.success('The professional was created');
        this.router.navigate(['/admin/professionals']);
      },
      error: () => {
        this.error.set(
          'The professional could not be created. Review unique email and phone values, then try again.',
        );
