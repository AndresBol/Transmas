...
export class ProfessionalAdminList {
  ...
  loadProfessionals(): void {
    ...
    this.professionalService.list().subscribe({
      ...
      error: () => {
        this.error.set('Professional administration could not be loaded.');
        this.loading.set(false);
      },
    });
  }
  ...
  confirmAvailability(profile: ProfessionalProfile): void {
    ...
    const data: ConfirmationDialogData = {
      ...
      message: isAvailable
        ? `Make ${name} available for requests again?`
        : `Mark ${name} as unavailable? The profile can be reactivated later.`,
    ...
    this.professionalService.updateAvailability(profile.id, isAvailable).subscribe({
      next: (response) => {
        this.professionals.update((items) =>
          items.map((item) => (item.id === profile.id ? { ...item, ...response.data } : item)),
        );
        this.updatingId.set(null);
        this.notificationService.success(
          isAvailable
            ? 'The professional is available again'
            : 'The professional is now unavailable',
        );
      },
      error: () => {
        this.updatingId.set(null);
        this.notificationService.error('The professional availability could not be updated');
      },
    });
  }
  ...
  canChangeAvailability(profile: ProfessionalProfile): boolean {
    return this.professionalService.hasUsableAccount(profile);
  }
  ...
  availabilityActionLabel(profile: ProfessionalProfile): string {
    if (profile.isActive === false)
      return 'Deleted profiles cannot change marketplace availability';
    if (profile.professional?.isActive === false)
      return 'Deleted accounts cannot change marketplace availability';
    if (profile.professional?.isBlocked)
      return 'Unblock the user account before changing marketplace availability';
