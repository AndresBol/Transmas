...
export class ProfessionalEditPage {
  ...
  loadFormData(): void {
    ...
    forkJoin({
      ...
      error: () => {
        this.error.set('The professional information could not be loaded.');
        this.loading.set(false);
      },
    });
  }
  ...
  save(data: ProfessionalCreateDto | ProfessionalUpdateDto): void {
    ...
    this.professionalService.update(this.id, data as ProfessionalUpdateDto).subscribe({
      next: () => {
        this.notificationService.success('The professional was updated');
        this.router.navigate(['/admin/professionals']);
      },
      error: () => {
        this.error.set(
          'The professional could not be updated. Review unique email and phone values, then try again.',
        );
