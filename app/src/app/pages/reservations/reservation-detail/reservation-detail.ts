...
export class ReservationDetail implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      this.error.set('The reservation reference is invalid.');
      return;
    }
    this.load(id);
  }
  load(id: number): void {
    ...
    this.service.getById(id).subscribe({
      ...
      error: () => {
        this.error.set('The reservation details could not be loaded.');
        this.loading.set(false);
      },
    });
  }
  fullName(reservation: Reservation): string {
  ...
  professionalName(reservation: Reservation): string {
    ...
    return profile?.professional
      ...
      : (profile?.professionalTitle ?? 'Professional unavailable');
  }
  districtLabel(district: District): string {
    return [district.name, district.canton?.name, district.canton?.province?.name]
      .filter(Boolean)
      .join(', ');
  }
